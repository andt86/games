...
#	BANNER("CLEAR BOARD",ff.GREEN)
	...
	for _ in range(3):
		BANNER("CLEARING BOARD",ff.RED)
		sleep(0.125)
		CLEAR()
		sleep(0.125)
	
	BANNER("BOARD CLEARED",ff.GREEN)
...
#			input()
	...
	def PC_turn(self, positions_list):
		DICT = self.position_DICT
		player_key = 2
		position_ = int(random.choice(positions_list))
		BOARD_grid.board[DICT[position_]] = str(player_key)
	...
	def game_player(self):
		BANNER("PLAYER plays 1st", ff.CYAN)
		input(str(c3+str("START :   ").rjust(12," ")+r))
		
		positions_list = [1,2,3,4,5,6,7,8,9]
#		self.clear_BOARD()
		take_turn = self.take_turn
		PC_turn = self.PC_turn
...
#			print_board(1)
#			input("    take turn  :  ")
			positions_ = take_turn(1, turn_cnt)
			positions_list.pop(positions_list.index(positions_))
...
#			input("    PC's' turn  :  ")
			...
			positions_ = PC_turn(positions_list)
			positions_list.pop(positions_list.index(positions_))
